...
import { deletdTodo, toggleComplatedTodo } from "../redux/slices/todoSlice";


const TodoItem = ({name , compalted,id}) => {
  ...
  return (
    ...
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-200 p-4 mb-3 border border-gray-100">
        ...
        <div className="flex justify-between items-center gap-3">
          <button 
          className="text-xs font-semibold px-4 py-2 rounded-lg bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-105 active:scale-95"
            onClick={()=>dispatch(toggleComplatedTodo(id))}
          >
            Complete
          </button>
          <div
          ...
          <button className="text-xs font-semibold px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-rose-500 hover:from-red-600 hover:to-rose-600 text-white cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-105 active:scale-95"
            onClick={()=>dispatch(deletdTodo(id))}
          >
            Delete
          </button>
        </div>
