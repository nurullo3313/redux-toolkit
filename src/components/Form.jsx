...
import { setNameTodo } from '../redux/slices/todoSlice'

export default function Form() {

    const dispath =useDispatch()
    const todoName = useSelector((state)=>state.todo.nameTodo)
    

    function addTodoHendler(){
        const todo = {
            id : Math.random()*1000,
            name : todoName,
            complated : false
  ...
  return (
      <form className='w-full flex gap-2 mb-4' onSubmit={(e) => e.preventDefault()}>
            <input
                ...
                onChange={(e)=>dispath(setNameTodo(e.target.value))}
                value={todoName}
            />
            <button
                type='submit'
